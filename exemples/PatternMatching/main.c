...
int main(int argc, char** argv)
{
    ...
    unsigned long results_count = 1;    // total number of results expected 
    ...
    err |= clSetKernelArg(kernel, 4, sizeof(cl_mem),		&results_output);
    if (err != CL_SUCCESS)
    {
    ...
    if(options.verify)
    {
        ...
        Info("Found '%lu' out of '%lu' expected patterns!\n", (unsigned long)found, (unsigned long)results[0]);
        Info(SEPARATOR);
    }
